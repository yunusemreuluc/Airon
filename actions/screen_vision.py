...
def _capture_active_window() -> tuple[bool, str, str]:
    ...
    try:
        handle = tempfile.NamedTemporaryFile(
            prefix="airon-screen-", suffix=".png", delete=False
        )
...
def _vision_prompt(query: str, window_title: str) -> str:
    ...
    return (
        "Sen Windows üzerinde Aıron için ekran analizi yapan bir görüntü yorumlayıcısısın.\n"
...
def analyze_screen(query: str, target: str = "active_window") -> dict:
    if not HAS_MSS:
        return fail(
    ...
    cap_ok, result, window_title = _capture_active_window()
    if not cap_ok:
        return fail(f"Ekran görüntüsü alınamadı: {result}")

    image_path = Path(result)
    try:
        if not image_path.exists():
            return fail("Ekran görüntüsü dosyası bulunamadı. Tekrar dene.")
        if image_path.stat().st_size <= 0:
            return fail("Ekran görüntüsü boş geldi.")
        if _image_looks_blank(image_path):
            return fail("Ekran görüntüsü siyah veya boş görünüyor.")
        ...
        try:
            ...
            if prefix:
                return fail(f"Ekran görüntüsü alındı ({prefix}) ama analiz tamamlanamadı: {exc}")
            return fail(f"Ekran görüntüsü alındı ama analiz tamamlanamadı: {exc}")

        if window_title:
            return ok(f"[Aktif pencere: {window_title}]\n{analysis}", window_title=window_title)
        return ok(analysis)
    finally:
        try:
